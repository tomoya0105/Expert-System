...
# # ファイル名を変数に格納する
# file1 = 'ShareWordFrequencyList_1.txt'
# file2 = 'ShareWordFrequencyList_2.txt'

# # 順序を保持した辞書型を作成する
# reSWFL1_dict = {}
# for word, count in reSWFL1:
#     if word in reSWFL1_dict:
#         reSWFL1_dict[word] += count
#     else:
#         reSWFL1_dict[word] = count

# reSWFL2_dict = {}
# for word, count in reSWFL2:
#     if word in reSWFL2_dict:
#         reSWFL2_dict[word] += count
#     else:
#         reSWFL2_dict[word] = count

# # 辞書型の要素を文字列に変換する
# reSWFL1_str = os.linesep.join([str(word) + ' ' + str(count) for word, count in reSWFL1_dict.items()])
# reSWFL2_str = os.linesep.join([str(word) + ' ' + str(count) for word, count in reSWFL2_dict.items()])

# # テキストファイルに書き込む
# with open(file1, 'w') as f1:
#     f1.write(reSWFL1_str)

# with open(file2, 'w') as f2:
#     f2.write(reSWFL2_str)